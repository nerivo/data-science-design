...
\hypersetup{
  pdftitle={Designing and Building Data Science Solutions: A Practical Guide},
  pdfauthor={Jonathan Leslie and Neri Van Otten},
  hidelinks,
...
{\centering \includegraphics[width=0.8\linewidth]{designing_and_building_data_science_solutions_files/figure-latex/nice-fig-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{include_graphics}\NormalTok{(}\KeywordTok{rep}\NormalTok{(}\StringTok{'figures/Figure_1-eval_levels.png'}\NormalTok{, }\DecValTok{1}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\includegraphics[width=0.7\linewidth]{figures/Figure_1-eval_levels}
...
For a more detailed discussion of this topic, we recommend this (\url{https://www.oreilly.com/radar/what-you-need-to-know-about-product-management-for-ai/}) piece by Peter Skomoroch and Mike Loukides.
...
\textbf{\textless start text box, ``OKRs and business goals''\textgreater{}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{include_graphics}\NormalTok{(}\KeywordTok{rep}\NormalTok{(}\StringTok{'figures/phases-flow.png'}\NormalTok{, }\DecValTok{1}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{include_graphics}\NormalTok{(}\KeywordTok{rep}\NormalTok{(}\StringTok{'figures/Framework phases.png'}\NormalTok{, }\DecValTok{1}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
We also encourage you to think about how you intend to structure your codebase. We generally use Git and GitHub for version control and code sharing, and we would encourage you to build your repository's directory structure before any code is added. We highly recommend the ``cookiecutter'' family of project templates (\url{https://github.com/cookiecutter/cookiecutter}) and have found the Python-based data science template (\url{http://drivendata.github.io/cookiecutter-data-science/}) to be a good fit for most of our Python-based projects. It includes a guide to best practices for directory structure and naming conventions and contains a built-in make functionality that can be useful for managing the steps required to build the requisite datasets and functions for your project. For R-based projects, one option is the ProjectTemplate (\url{http://projecttemplate.net/}), although others also exist. At the minimum, we encourage you to work within the framework of an R Project (\url{https://support.rstudio.com/hc/en-us/articles/200526207-Using-Projects}).
...
A note on notes\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.13\columnwidth}\raggedright
Any experience researcher will know that a critical part of the work is in keeping detailed records. This is no different for the data scientist -- keeping track of results, as well as detailed records of assumptions and choices that you make, is essential. Aside from the fact that this is scientific best-practice, it also makes practical sense: it is not uncommon to examine your final results and realise that something doesn't make intuitive sense. In this case, you will have to do some detective work to figure out why. Good notes that include information about the assumptions and potential errors you made along the way will make that task a lot easier.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.13\columnwidth}\raggedright
...
\hypertarget{listen-to-people}{%
\subsection{Listen to people}\label{listen-to-people}}
